...
def shape_element(element):
    ...
    if element.tag == "node" or element.tag == "way" :
        ...
        for tag in element.iter('tag'):
            ...
            if p:
                continue
            elif is_address(tag): #addr is present 
                if ':' in tag.attrib['k'][5:]: #checking for second :
                    continue
                else: #first colon processing
                    after_colon = tag.attrib['k'][5:]
                    if after_colon == 'postcode':
                        address_info[tag.attrib['k'][5:]] = update_postcode(tag.attrib['v'])
                    else :
                        address_info[tag.attrib['k'][5:]]=update_name(tag.attrib['v'], mapping)
            
            # if addr is present -- then you get string after the : using [5:]
            # take the string after : in a variable and check if that's postcode 
            # if yes then update_postcode 
            # else update update_name
